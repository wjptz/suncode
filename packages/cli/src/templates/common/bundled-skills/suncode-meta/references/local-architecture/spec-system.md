...
`.trellis/spec/` is the user's project-specific engineering spec library. Suncode is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
| Change when specs should be updated | Phase 3.3 in `.trellis/workflow.md` and the `suncode-update-spec` skill. |

## Boundaries

`.trellis/spec/` is the user's project specification, not a permanent copy of Suncode built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating Suncode default templates as immutable documents.
