...
def _has_subagent_platform(repo_root: Path) -> bool:
    ...
    only counts when ``codex.dispatch_mode`` explicitly opts into ``auto``
    (including the legacy ``sub-agent`` alias); inline mode loads context
    through skills, not JSONL.
    """
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # Guard against a full task directory name pasted into --slug. Only an
    # explicit slug is normalized; title-derived slugs remain untouched.
    if args.slug:
        match = re.match(r"^(\d{2})-(\d{2})-(.+)$", slug)
        if match and 1 <= int(match.group(1)) <= 12 and 1 <= int(match.group(2)) <= 31:
            slug_prefix = f"{match.group(1)}-{match.group(2)}"
            if slug_prefix == date_prefix:
                ...
                print(f"Pass only the slug body, e.g. --slug {match.group(3)}", file=sys.stderr)
                return 1
...
def _auto_commit_archive(
    ...
    ``git add -f .suncode/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.suncode/config.yaml``: when
