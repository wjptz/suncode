...
`suncode channel messages <channel>` renders a compact, human-readable view:
...
suncode channel messages <channel> --kind done --last 10
suncode channel messages <channel> --kind error --last 10

# Raw (diagnostic view) — one JSON per line
suncode channel messages <channel> --raw --kind progress --last 20
suncode channel messages <channel> --raw --last 50
```
...
```bash
suncode channel messages <channel> --raw --kind progress --last 80 \
...
Symptom: `suncode channel list` shows the worker as running, but no new
...
2. **Confirm the supervisor and worker PIDs are alive.**
   ...
   `suncode channel kill <name> --as <worker> --force`.
...
```bash
suncode channel wait T --as main --from check --kind done --timeout 15m
suncode channel wait T --as main --from check,check-cx --kind done --all --timeout 15m
suncode channel wait T --as worker --tag interrupt --timeout 1h
suncode channel wait T --as main --thread release-note --action status --timeout 10m
```
...
suncode channel forum list <channel>

# Inspect one thread end-to-end
suncode channel thread show <channel> <thread>

# Replay messages for a thread (supports --raw, --kind, --last)
suncode channel messages <channel> --thread <thread> --raw --last 100

# What a specific worker still has pending
suncode channel context <channel> --as <worker>
```
...
| ghost worker in list | supervisor died without cleanup | `suncode channel kill <name> --as <worker> --force` |
