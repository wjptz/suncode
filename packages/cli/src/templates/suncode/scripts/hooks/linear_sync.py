...
Manual usage:
    TASK_JSON_PATH=.suncode/tasks/<name>/task.json python3 .suncode/scripts/hooks/linear_sync.py sync

Environment:
    TASK_JSON_PATH  - Absolute path to task.json (set by task.py)

Configuration:
    .suncode/hooks.local.json  - Local config (gitignored), example:
    {
      "linear": {
        "team": "TEAM_KEY",
...
def _load_config() -> dict:
    """Load local hook config from .suncode/hooks.local.json."""
    task_json_path = os.environ.get("TASK_JSON_PATH", "")
    if task_json_path:
        # Walk up from task.json to find .suncode/
        suncode_dir = Path(task_json_path).parent.parent.parent
    else:
        suncode_dir = Path(".suncode")

    config_path = suncode_dir / "hooks.local.json"
    try:
