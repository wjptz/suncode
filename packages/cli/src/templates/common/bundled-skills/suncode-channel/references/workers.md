...
- `--jsonl <path>` — repeatable Suncode manifest where every line is
...
```bash
suncode channel spawn cr-feature --agent check --as check-claude
suncode channel spawn cr-feature --agent check --provider codex --as check-cx

suncode channel wait cr-feature --as main \
...
echo "Stop refactoring the parser — switch to fixing the failing test in src/foo.ts" \
  | suncode channel interrupt impl-task --as dispatcher --to codex-impl --stdin
```
...
echo "Check this when you reach the next turn." \
  | suncode channel send impl-task --as dispatcher --to codex-impl \
      --stdin --tag question
```
...
suncode channel wait impl-task --as dispatcher \
  --from codex-impl --kind done,error --timeout 30m

# 3. Read the final answer.
suncode channel messages impl-task --from codex-impl --last 1 --raw
```
