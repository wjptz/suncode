...
Usage:
    ...
    # Structured content (repeatable; empty sections are omitted):
...
def is_git_worktree(repo_root: Path) -> bool:
    """Return whether repo_root is a linked worktree, not the main checkout."""
    ...
    if rc_dir != 0 or rc_common != 0:
        return False
    return (repo_root / git_dir.strip()).resolve() != (
        repo_root / git_common_dir.strip()
    ).resolve()


def warn_if_parallel_worktree(repo_root: Path) -> None:
    """Explain the expected index conflict without blocking journal writes."""
    if not get_session_auto_commit(repo_root) or not is_git_worktree(repo_root):
        return
    print(
        colored(
            ...
            "not index.md). See .suncode/spec/cli/backend/directory-structure.md "
...
def _render_bullet_section(
    ...
    """Render a Markdown bullet section, omitting it when empty."""
    if not items:
        return ""
...
def _render_main_changes(
    changes: list[str],
    extra_content: str | None,
) -> str:
    """Prefer structured changes, falling back to free-form content."""
    if changes:
...
|------|---------|"""
    ...
    testing_section = _render_bullet_section(
        "Testing", tests or [], bullet_prefix="- [OK] "
    )
...
def _auto_commit_workspace(repo_root: Path) -> None:
    ...
    via ``get_current_task``). We never `git add` the whole `.suncode/` tree
    ...
    Honors ``session_auto_commit`` in ``.suncode/config.yaml``: when set to
    ...
    # is ``.suncode/tasks/<name>`` (or under archive/) — pass the bare name.
    current = get_current_task(repo_root)
    if current:
        task_name = Path(current).name
        paths = safe_suncode_paths_to_add(repo_root, task_name=task_name)
    else:
        ...
        paths = [
            p
            for p in safe_suncode_paths_to_add(repo_root, task_name=None)
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary",
        default="Session summary was not supplied.",
        help="Brief summary",
    )
