 ...
 * - `.snow/commands/suncode-*.json` — custom prompt slash commands (no suncode-start)
 ...
 *
 * Modern Snow does NOT ship `.snow/sub-agents.suncode.json` (legacy merge
...
function buildSnowCommandJson(name: string, content: string): string {
  const description =
    name === "continue"
      ? "Resume the current Suncode task at the right workflow phase."
      : name === "finish-work"
        ? "Wrap up the current Suncode session: archive tasks and record journal."
        : `Suncode: ${name}`;
...
/**
 * Collect all Snow template files for `suncode update` diff tracking.
...
export function collectSnowTemplates(): Map<string, string> {
  ...
  // hasHooks=true → resolveAllAsSkills drops suncode-start.
  ...
  for (const cmd of resolveCommands(ctx)) {
    ...
    files.set(
      `.snow/commands/suncode-${cmd.name}.json`,
...
export async function configureSnow(cwd: string): Promise<void> {
  ...
  for (const cmd of resolveCommands(ctx)) {
    const body = replacePythonCommandLiterals(cmd.content);
    await writeFile(
      path.join(commandsDir, `suncode-${cmd.name}.json`),
      buildSnowCommandJson(cmd.name, body),
    );
  }
