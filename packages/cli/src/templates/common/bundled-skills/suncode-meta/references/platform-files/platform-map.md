...
This page lists common Suncode file locations in a user project by platform. Whether a platform directory exists in an actual project depends on which `suncode init --<platform>` commands the user ran.
...
### Suncode Sub-Agent Support

These platforms usually have `suncode-research`, `suncode-implement`, and `suncode-check` files:
...
When changing behavior, inspect workflows and skills first. Do not assume Suncode sub-agents exist.
