...
You are already the `suncode-implement` sub-agent that the main session dispatched.
Do the implementation work directly.

- Do NOT spawn another `suncode-implement` or `suncode-check` sub-agent via the `task` tool.
...
- Only the main session may dispatch Suncode implement/check agents. If more parallel work
