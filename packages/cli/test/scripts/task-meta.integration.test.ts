 ...
 * `src/templates/suncode/scripts/task.py` (subcommand wiring) and
 * `src/templates/suncode/scripts/common/task_store.py` (`cmd_create` /
 * `cmd_set_meta`); this test stamps the real templates into a fresh
 * `.suncode/` tree and exercises the actual CLI paths.
 */
...
function runTask(repo: string, ...args: string[]) {
  return spawnSync("python3", [".suncode/scripts/task.py", ...args], {
    cwd: repo,
    encoding: "utf-8",
  });
}
...
function readTaskJson(repo: string, dirName: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repo, ".suncode", "tasks", dirName, "task.json"),
      "utf-8",
    ),
  );
}
...
function findTaskDir(repo: string, needle: string): string {
  const dir = fs
    .readdirSync(path.join(repo, ".suncode", "tasks"))
...
describe.skipIf(!hasPython())("task.py meta (task.json.meta access)", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "suncode-task-meta-test-"));
    setupRepo(tmp);
  });
  ...
  it("create --meta with a malformed value errors and names the bad value", () => {
    ...
    const dirs = fs.existsSync(path.join(tmp, ".suncode", "tasks"))
      ? fs.readdirSync(path.join(tmp, ".suncode", "tasks"))
      : [];
  ...
  it("set-meta adds a new key and overwrites an existing one", () => {
    ...
    const taskDir = `.suncode/tasks/${dir}`;

    const original = readTaskJson(tmp, dir);
    ...
    fs.writeFileSync(
      path.join(tmp, ".suncode", "tasks", dir, "task.json"),
    ...
    expect(readTaskJson(tmp, dir).meta).toMatchObject({
      ...
      execution: originalMeta.execution,
      hub: originalMeta.hub,
    });
