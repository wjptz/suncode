...
Snow is a **class-1** Suncode host: auto context inject + project agent discovery +
`beforeSubAgentStart` prompt enrichment.

| Capability                                        | Status                                                                       |
| ------------------------------------------------- | ---------------------------------------------------------------------------- |
| Skills (`.snow/skills/suncode-*/SKILL.md`)        | Works                                                                        |
| Prompt commands (`.snow/commands/suncode-*.json`) | Works (`/suncode-continue`, `/suncode-finish-work`, …)                       |
| Context hooks (`.snow/hooks/`)                    | Inject model context via stdout JSON + write `.snow/log/suncode-context.txt` |
| Project agents (`.snow/agents/*.md`)              | Auto-discovered by Snow (`#suncode-implement`, …)                            |
| `beforeSubAgentStart`                             | Injects active-task breadcrumb into sub-agent prompts                        |
| `suncode-start`                                   | Optional — session hooks replace the old manual ritual                       |

## Quick start

```bash
suncode init --snow -u your-name
snow
```

In Snow:

1. Open a session in the project root — `onSessionStart` injects Suncode context automatically.
2. Dispatch implement/check/research (project agents under `.snow/agents/`). Prefer prompt first line:

```text
Active task: .suncode/tasks/<id>
```

3. Optional: `/suncode-continue` / `/suncode-finish-work`, or `skill-execute` on `suncode-*` skills.
...
- exit 1 on `onUserMessage` → replace (not used by Suncode)
...
| `SUNCODE_CONTEXT_ID` | `snow-c2343752-...` | Preferred Suncode active-task context key |
| `SNOW_CWD`           | project root        | Working directory for hooks/tools         |
| `SNOW_PLATFORM`      | `snow`              | Platform tag                              |

Notes:

- `SUNCODE_CONTEXT_ID` wins when already set (explicit override).
- Otherwise Suncode resolves `SNOW_SESSION_ID` via `active_task.py` as platform `snow`.
