 ...
 *   - `src/templates/suncode/scripts/common/config.py` — config parsing
 ...
 *   - `src/templates/suncode/scripts/common/task_context.py` — `task.py
...
function makeTask(tmp: string, dirName: string): string {
  const taskDir = path.join(tmp, ".suncode", "tasks", dirName);
...
describe.skipIf(!hasPython())("context injection limits (issue #441)", () => {
  let tmp: string;

  beforeEach(() => {
    tmp = fs.mkdtempSync(
      path.join(os.tmpdir(), "suncode-context-limits-test-"),
    );
    setupRepo(tmp);
  });
...
`,
  ...
  describe("inject-subagent-context.py: per-file and per-artifact caps", () => {
    it("under-cap content is byte-identical to unlimited output (golden)", () => {
      ...
      expect(out).not.toContain("[Suncode: truncated");
      expect(out).not.toContain("[Suncode: not inlined");
    });
    ...
    it("keeps binary jsonl references as notices even when limits are unlimited", () => {
      ...
      expect(out).toContain(
        "[Suncode: not inlined (binary file) — design.png (10 bytes): visual baseline]",
      );
      expect(out).toContain(
        "[Suncode: not inlined (binary file) — invalid.bin (3 bytes): legacy export]",
      );
    ...
    it("does not misclassify legitimate multi-byte UTF-8 content as binary", () => {
      ...
      expect(out).not.toContain("[Suncode: not inlined (binary file)");
    });
    ...
    it("classifies a file as binary when binary bytes appear only at the end", () => {
      ...
      expect(out).toContain(
        `[Suncode: not inlined (binary file) — mixed.dat (${mixed.length} bytes): mixed content]`,
      );
    ...
    it("truncates an oversized jsonl-referenced file at max_file_bytes with a notice", () => {
      ...
      expect(out).toContain(
        "[Suncode: truncated at 32768 bytes — read big.txt for the full content]",
      );
    });

    it("bounds detailed notices with one terminal omission notice", () => {
      const taskDir = makeTask(tmp, "task-total-cap");
      fs.writeFileSync(path.join(tmp, "f1.txt"), "1".repeat(50), "utf-8");
      fs.writeFileSync(path.join(tmp, "f2.txt"), "2".repeat(100), "utf-8");
      ...
      writeConfig(
        ...
        [
          ...
          "  max_total_bytes: 150", // fits f1 + f2 index; f3 triggers one terminal notice
        ].join("\n"),
      );
      const relTask = path.relative(tmp, taskDir).split(path.sep).join("/");
      const out = runHookProbe(
      ...
      expect(out).toContain(
        "[Suncode: not inlined (total context limit reached) — f2.txt (100 bytes): second]",
      );
      expect(out).toContain(
      ...
      expect(out).not.toContain("=== f3.txt ===");
      expect(
        out.match(/remaining context entries omitted/g) ?? [],
      ).toHaveLength(1);
      expect(Buffer.byteLength(out, "utf-8")).toBeLessThanOrEqual(230);
    });
    ...
    it("directory entries respect the per-file cap and total budget", () => {
      ...
      expect(out).toContain(
        "[Suncode: truncated at 10 bytes — read refdir/a.md for the full content]",
      );
      expect(out).toContain(
        "[Suncode: truncated at 10 bytes — read refdir/b.md for the full content]",
      );
    });
    ...
    it("artifacts (prd/design/implement.md) obey max_artifact_bytes independently of max_file_bytes", () => {
      ...
      fs.writeFileSync(path.join(taskDir, "prd.md"), "P".repeat(1000), "utf-8");
      writeConfig(
        tmp,
        [
          "context_injection:",
      ...
      expect(out).toContain(
        `[Suncode: truncated at 20 bytes — read ${relTask}/prd.md for the full content]`,
      );
    });
  });

  describe("task.py validate: JSONL hygiene warnings", () => {
    function runValidate(taskDir: string): {
      status: number | null;
      stdout: string;
      stderr: string;
    } {
      const r = spawnSync(
        "python3",
        [path.join(tmp, ".suncode", "scripts", "task.py"), "validate", taskDir],
    ...
    it("does not warn about a code-looking path under .suncode/spec/, docs/, or the task's own directory", () => {
      const taskDir = makeTask(tmp, "task-code-exempt");
      fs.mkdirSync(path.join(tmp, ".suncode", "spec"), { recursive: true });
      fs.writeFileSync(
        path.join(tmp, ".suncode", "spec", "example.py"),
        "# example only\n",
        "utf-8",
      );
      fs.writeFileSync(
    ...
    it("stays warning-free for a clean, under-cap, spec-only manifest", () => {
      ...
      fs.mkdirSync(path.join(tmp, ".suncode", "spec"), { recursive: true });
      fs.writeFileSync(
        path.join(tmp, ".suncode", "spec", "guide.md"),
        "spec guide\n",
        "utf-8",
      );
      fs.writeFileSync(
