...
Extracts step-level content from .suncode/workflow.md and optionally filters
...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    or ``"codex-sub-agent"`` when ``.suncode/config.yaml`` explicitly sets
    ...
    context injection makes explicit ``auto`` safe without changing that
    Suncode product default.
