...
def _parse_active_task_path(current_out: str, repo: Path) -> Path | None:
    # Prefer "Current task: .suncode/tasks/..." line from task.py --source.
    ...
    m = re.search(r"(\.suncode[/\\]tasks[/\\][^\s]+)", current_out)
    if m:
    ...
    for arg in argv[1:]:
        ...
        if a in {"session", "user", "subagent", "full", "compact"}:
            ...
            if a == "compact":
                return "user"
            return a
    env_mode = (env.get("SUNCODE_SNOW_HOOK_MODE") or "").strip().lower()
...
def _current_session_ids(stdin_ctx: dict[str, Any] | None = None) -> list[str]:
    # Prefer explicit Suncode context, then Snow session identity.
    ids: list[str] = []
    candidates = [
        os.environ.get("SUNCODE_CONTEXT_ID"),
    ...
    for raw in candidates:
        ...
        if key and key not in ids:
            ids.append(key)
        # Snow injects SUNCODE_CONTEXT_ID=snow-<sessionId>
...
def _resolve_runtime_session_file(
    ...
    sessions_dir = repo / ".suncode" / ".runtime" / "sessions"
...
def _workflow_phase_summary(
    ...
    lines: list[str] = []
    workflow = repo / ".suncode" / "workflow.md"
    if workflow.is_file():
    ...
    session_md = repo / ".suncode" / "session" / "current.md"
    if session_md.is_file():
        body = _read_text(session_md, 1200)
        if body:
            lines.extend(["## .suncode/session/current.md", body, ""])
    else:
...
def build_context(
    ...
    *,
    ...
    task_py = repo / ".suncode" / "scripts" / "task.py"
    current = ""
    ...
    if task_py.is_file():
        ...
        lines.append("(no .suncode/scripts/task.py — run suncode init first)")
        lines.append("")
    ...
    if not compact:
        ...
        identity = repo / ".suncode" / "identity.md"
        if identity.is_file():
            body = _read_text(identity, 900 if mode == "subagent" else 1200)
            if body:
                lines.extend(["## .suncode/identity.md", body, ""])
    else:
    ...
    if mode == "subagent" and kind == "research":
        ...
        lines.extend(
            [
                ...
                "1. Session/user hooks auto-inject Suncode context; `/suncode-start` is optional."
                if not compact
                else "1. Compact inject — full context also at `.snow/log/suncode-context.txt`.",
                "2. When dispatching implement/check/research, start the prompt with:",
                "   `Active task: <path from task.py current>`",
                "3. Do not git commit/push/merge from Suncode implement/check agents.",
                "",
            ]
        )
...
def main() -> int:
    try:
        ...
        try:
            ...
            # Keep suncode-context.txt as the richest practical breadcrumb for
            # pull-based reads. User-mode inject stays compact, but must not
            # clobber a prior full snapshot.
            log_path = log_dir / "suncode-context.txt"
            if mode == "user":
            ...
            sys.stderr.write(f"suncode-context write failed: {exc}\n")

        max_bytes = COMPACT_MAX_BYTES if mode == "user" else DEFAULT_MAX_BYTES
        inject = _truncate(context, max_bytes)
        display = {
            "session": "Suncode session context injected",
            "user": "Suncode compact breadcrumb",
            "subagent": f"Suncode sub-agent context ({_agent_kind(stdin_ctx)})",
        }.get(mode, "Suncode context refreshed")
        display = f"{display} (.snow/log/suncode-context.txt)"
        ...
        try:
            sys.stderr.write(f"write-suncode-context failed: {exc}\n")
            # Still emit empty-safe JSON so host can continue.
            sys.stdout.write(
                json.dumps(
                    {
                        "additionalContext": "# Suncode context unavailable (hook error)\n",
                        "display": "Suncode context hook error (fail-open)",
                    },
                    ensure_ascii=False,
                )
            )
