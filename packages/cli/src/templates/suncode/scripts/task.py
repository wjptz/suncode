...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.suncode/tasks/01-31-my-task')")
        return 1
    ...
    if not resolve_context_key():
        ...
        print(colored(
            ...
            "or set SUNCODE_CONTEXT_ID before running task.py start.",
            Colors.YELLOW,
        ))
...
Examples:
  ...
  python3 task.py start .suncode/tasks/01-21-add-login
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print("See .suncode/workflow.md planning artifact guidance or run:", file=sys.stderr)
        print(
            "  python3 ./.suncode/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
