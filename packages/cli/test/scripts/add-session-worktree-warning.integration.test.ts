...
function setupRepo(tmp: string, autoCommit: boolean): void {
  ...
  // .suncode/.developer, .current-task, and .runtime/ are gitignored by
...
function runAddSession(
  ...
  const r = spawnSync(
    "python3",
    [".suncode/scripts/add_session.py", "--title", "parallel session", "--no-commit"],
...
describe.skipIf(!hasPython())("add_session.py worktree warning", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "suncode-worktree-warn-"));
    worktreeDir = null;
  });
  ...
  it("warns once when running inside a linked worktree with auto-commit enabled", () => {
    ...
    worktreeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "suncode-worktree-warn-linked-"),
    );
    ...
    fs.writeFileSync(
      path.join(worktreeDir, ".suncode", ".developer"),
      `name=${DEVELOPER}\n`,
    );
    ...
    const r = spawnSync(
      "python3",
      [
        ".suncode/scripts/add_session.py",
        "--title",
        "parallel session",
        "--no-commit",
      ],
  ...
  it("stays silent in a linked worktree when session_auto_commit is false", () => {
    ...
    worktreeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "suncode-worktree-warn-off-"),
    );
    fs.rmdirSync(worktreeDir);
