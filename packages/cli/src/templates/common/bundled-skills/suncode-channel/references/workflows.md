...
```bash
suncode channel create brainstorm-storage-layer --by main \
  --task .suncode/tasks/05-XX-storage-adapter

suncode channel spawn brainstorm-storage-layer \
  --agent architect --provider codex \
  --file .suncode/tasks/05-XX-storage-adapter/prd.md \
  --file .suncode/tasks/05-XX-storage-adapter/design.md \
  --as cx-arch --timeout 30m

suncode channel send brainstorm-storage-layer \
  --as main --to cx-arch --text-file /tmp/brainstorm-r1.md

suncode channel wait brainstorm-storage-layer \
...
```bash
TASK=.suncode/tasks/05-12-foo
suncode channel create cr-foo --task "$TASK" --by main

suncode channel spawn cr-foo \
  --agent check \
...
suncode channel send cr-foo --as main --to check --text-file /tmp/cr-brief.md
suncode channel wait cr-foo --as main --kind done --from check --timeout 15m
suncode channel messages cr-foo --kind message --from check --tag final_answer
```
...
```bash
suncode channel create cr-feature --by main --ephemeral

suncode channel spawn cr-feature --agent check \
  --jsonl "$TASK/check.jsonl" --file "$TASK/prd.md" --file "$TASK/design.md" \
  --timeout 15m

suncode channel spawn cr-feature --agent check --provider codex --as check-cx \
  --jsonl "$TASK/check.jsonl" --file "$TASK/prd.md" --file "$TASK/design.md" \
  --timeout 15m

suncode channel send cr-feature --as main --to check --text-file /tmp/cr-brief.md
suncode channel send cr-feature --as main --to check-cx --text-file /tmp/cr-brief.md
suncode channel wait cr-feature --as main --kind done --from check,check-cx --all --timeout 15m
```
...
```bash
suncode channel run --provider codex --message "say hi in 3 words" --timeout 1m
suncode channel run --agent plan --message-file /tmp/plan-question.md --timeout 10m
```
...
```bash
suncode channel forum <board> --scope global
suncode channel thread <board> <thread> --scope global --raw
suncode channel context list <board> --scope global --thread <thread>
suncode channel messages <board> --scope global --raw --thread <thread>
```
