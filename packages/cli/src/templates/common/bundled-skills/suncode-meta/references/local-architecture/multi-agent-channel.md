...
2. **Agent definitions** inside the project at `.trellis/agents/`: platform-agnostic role cards consumed by `suncode channel spawn --agent <name>`.
...
| Override worker guard per spawn | Pass `--idle-timeout` / `--max-live-workers` on `suncode channel spawn`, or set `TRELLIS_CHANNEL_WORKER_IDLE_TIMEOUT` / `TRELLIS_CHANNEL_MAX_LIVE_WORKERS` in the environment. |
...
| Add a new role card | Drop `<name>.md` into `.trellis/agents/`. `suncode channel spawn --agent <name>` will pick it up. |
