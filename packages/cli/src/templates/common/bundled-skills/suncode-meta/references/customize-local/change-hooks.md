...
Hooks are the automation layer that connects a platform to Suncode. When the user wants to change "when context is injected," "how shell commands inherit a session," or "which files are read before an agent starts," hooks are usually the edit point.
...
| Hook | Purpose |
| --- | --- |
| session-start | Injects a Suncode overview when a session starts, clears, or compacts. |
