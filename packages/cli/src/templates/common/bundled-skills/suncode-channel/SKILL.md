---
name: suncode-channel
description: Use Suncode channel for live multi-agent collaboration, spawned workers, cross-agent review, progress inspection, forum channels, and channel log debugging.
---

# suncode-channel

`suncode channel` is the local multi-agent collaboration runtime. Reach for it when agents need to talk through a durable event log, when a worker should be spawned as a peer process, when an in-flight worker needs interrupt / debugging, or when feedback should be recorded on a durable `--type forum` channel.
...
suncode channel thread <board> <thread> --scope global
...
- Use `--context-file` / `--context-raw` and `suncode channel context add/delete/list`. `--linked-context-*` is deprecated terminology.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `suncode mem` (the `suncode-session-insight` skill) for session/history search.
