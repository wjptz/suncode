...
Authoritative current command reference for `suncode channel` subcommands,
...
```bash
suncode channel create <name>
  [--scope project|global]                # default: project
  [--type chat|forum]                     # default: chat
  [--task <path>]                         # associated Suncode task dir
  [--project <slug>]
  [--labels a,b,c]
...
```bash
suncode channel send <name> [text]
...
- `--kind` is the only event-type filter, and it is constrained to the
  Suncode-emitted whitelist (`CHANNEL_EVENT_KINDS` in
...
- Use `suncode channel interrupt` (the command) only when you actually want
...
```bash
suncode channel send T --as A --stdin < /tmp/message.md
suncode channel send T --as A --text-file /tmp/message.md
```
...
```bash
suncode channel interrupt <name> [text]
...
```bash
suncode channel post <name> <action>
  --as <agent>                            # REQUIRED
  [--scope project|global]
...
```bash
suncode channel thread <name> <thread-key>
  [--scope project|global]
  [--raw]

suncode channel thread rename <name> <old-thread> <new-thread>
  --as <agent>                            # REQUIRED
  [--scope project|global]
```
...
```bash
suncode channel context add <name>
...
suncode channel context delete <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
  [--thread <key>]
  [--file <abs-path>] ...
...
suncode channel context list <name>
  [--scope project|global]
  [--thread <key>]
...
```bash
suncode channel title set <name>
  --title <text>                          # REQUIRED
  [--as <agent>]                          # default: main
  [--scope project|global]

suncode channel title clear <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
```
