...
| [文件系统安全契约](./filesystem-safety.md) | Channel 上下文、JSONL、agent 文件的 realpath containment、可信根与符号链接边界 | Done |
...
| [`suncode platforms` Command](./commands-platforms.md) | Machine-readable report of configured AI platforms: `--json` shape, registry sourcing, and failure behavior | Done |
...
- Editing channel context/agent file reads, trusted roots, or `.suncode/tasks` / `.suncode/workspace` symlinks → [filesystem-safety.md](./filesystem-safety.md)
