# `suncode mem` CLI Reference

Full flag reference for the five subcommands. Pin this as the authoritative source — `suncode mem help` prints the same content at runtime, so anything here that drifts is a bug.
...
| `--phase brainstorm\|implement\|all`          | extract           | Slice session by Suncode task boundaries. `brainstorm` = `[task.py create, task.py start)`. `implement` = turns outside brainstorm windows. Default `all`. |
...
suncode mem search "deadlock" --global --limit 20

# Inside a specific session, surface the top 5 turns that mention "lock contention"
# plus 2 turns of surrounding context.
suncode mem context 5842592d --grep "lock contention" --turns 5 --around 2

# Recover the brainstorm window for a session — useful when continuing a task
# the user started a week ago.
suncode mem extract 5842592d --phase brainstorm

# List every project this machine has Suncode sessions for, with counts.
suncode mem projects
```
...
Run `suncode mem help` in the user's shell. The runtime help is authoritative and will be ahead of this reference during fast-moving beta releases.
