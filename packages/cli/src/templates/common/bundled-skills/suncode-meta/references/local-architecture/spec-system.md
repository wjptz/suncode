...
`.suncode/spec/` is the user's project-specific engineering spec library. Suncode is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
`.suncode/config.yaml` can declare packages:
...
```jsonl
{"file": ".suncode/spec/cli/backend/index.md", "reason": "CLI backend conventions"}
{"file": ".suncode/spec/cli/unit-test/conventions.md", "reason": "Test expectations"}
```
...
When the AI learns a new rule during implementation or debugging, it should update `.suncode/spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need | Edit location |
| --- | --- |
| Add a new spec layer | `.suncode/spec/<package>/<layer>/index.md` and corresponding guideline files. |
| Change monorepo spec mapping | `packages` / `default_package` / `spec_scope` in `.suncode/config.yaml`. |
...
| Change when specs should be updated | Phase 3.3 in `.suncode/workflow.md` and the `suncode-update-spec` skill. |

## Boundaries

`.suncode/spec/` is the user's project specification, not a permanent copy of Suncode built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating Suncode default templates as immutable documents.
