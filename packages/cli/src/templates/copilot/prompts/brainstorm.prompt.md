---
description: "Guide requirements discovery for a Suncode task after task-creation consent."
---
...
```bash
TASK_DIR=$({{PYTHON_CMD}} ./.suncode/scripts/task.py create "<short task title>" --slug <slug>)
```
...
After the requirement convergence gate, rewrite `prd.md` once against the final structure described in the artifact rules above. This is not optional cleanup; it prepares the latest artifacts for final review.
