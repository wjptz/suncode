...
```bash
suncode channel create design-feedback \
  --type forum \
  --scope global \
...
```bash
suncode channel post design-feedback opened \
  --scope global \
  --as main \
...
suncode channel post design-feedback comment \
  --scope global \
  --as reviewer \
  --thread login-empty-state \
  --text-file /tmp/review.md

suncode channel post design-feedback status \
  --scope global \
  --as main \
  --thread login-empty-state \
  --status closed

suncode channel post design-feedback summary \
  --scope global \
  --as main \
  --thread login-empty-state \
...
```bash
suncode channel messages design-feedback --scope global
suncode channel forum design-feedback --scope global --status open
suncode channel thread design-feedback login-empty-state --scope global
suncode channel messages design-feedback --scope global --raw --thread login-empty-state
```
...
```bash
suncode channel context list design-feedback --scope global
suncode channel context list design-feedback --scope global --thread login-empty-state --raw
```
...
```bash
suncode channel context delete design-feedback \
  --scope global \
  --thread login-empty-state \
  --raw "stale note"
```
...
```bash
suncode channel title set design-feedback \
  --scope global \
  --title "Design feedback board"

suncode channel title clear design-feedback --scope global
```
...
```bash
suncode channel thread rename design-feedback old-key new-key \
  --scope global \
  --as main
```
...
```bash
suncode channel create release-notes \
  --type forum \
  --scope global \
...
suncode channel post release-notes opened \
  --scope global \
  --as main \
