...
Write `.suncode/spec/` code-specs, guide updates, indexes, examples, explanations, and human-facing notes in Simplified Chinese by default. Keep English only where it is the original identifier or clearer technical literal, such as command names, API fields, code symbols, file paths, error strings, and quoted source text.
