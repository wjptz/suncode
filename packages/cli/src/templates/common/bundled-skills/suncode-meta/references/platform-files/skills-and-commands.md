...
Skills and commands are textual entry points for user interaction with Suncode. Different platforms use different names, but their core purpose is the same: tell the AI how to enter the Suncode flow when the user expresses a certain intent.
...
Suncode workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `suncode-meta` use layered references.
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `suncode-meta`.
