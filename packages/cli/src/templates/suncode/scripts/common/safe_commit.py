...
----------------------
A real user incident: a project's `.gitignore` listed `.suncode/` (company-wide
...
`git add -f .suncode/` — which fan-out-included every ignored subtree
(`.suncode/.backup-*/`, `.suncode/worktrees/`, `.suncode/.template-hashes.json`,
`.suncode/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.suncode/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.suncode/` in `.gitignore` is treated as user
  intent ("keep .suncode/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .suncode/` ...``
...
# Paths under .suncode/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.suncode/` tree.
SUNCODE_IGNORED_SUBPATHS = (
    ".suncode/.backup-*",
    ".suncode/worktrees/",
    ".suncode/.template-hashes.json",
    ".suncode/.runtime/",
    ".suncode/.cache/",
)


def safe_suncode_paths_to_add(
    repo_root: Path,
    ...
    Included:
      - .suncode/workspace/<developer>/journal-*.md
      - .suncode/workspace/<developer>/index.md
      - .suncode/tasks/<task_name>/   (ONLY the current task dir when
    ...
    Excluded (intentionally — these must not be staged):
      - .suncode/.backup-*, .suncode/worktrees/,
        .suncode/.template-hashes.json, .suncode/.runtime/, .suncode/.cache/
...
def safe_archive_paths_to_add(
    ...
    `.suncode/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .suncode/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .suncode/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    if paths:
        ...
        print(
            "[WARN]   .suncode/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .suncode/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .suncode/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.suncode/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    for sub in SUNCODE_IGNORED_SUBPATHS:
        print(f"[WARN]   {sub}", file=sys.stderr)
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .suncode/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .suncode/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .suncode/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
